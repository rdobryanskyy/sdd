...
4. **Core mapping (the contract of this skill).** Build the AC→test table: **every acceptance criterion in §5 maps to ≥1 test.** One criterion may fan out to several rows (a unit test for the rule plus an e2e test for the flow). A criterion with zero tests is the cardinal anti-pattern — the spec is then unverifiable. Name each test descriptively from the criterion's intent (e.g. `over-quota request is rejected`), not from any framework convention.
