...
3. **Devil's-advocate subagent (the core mechanic).** Dispatch one `Agent` (`subagent_type: "general-purpose"`, clean context — it never saw this conversation) with the prompt body in [`./references/ambiguity-checks.md`](./references/ambiguity-checks.md). It Reads `spec.md` (and `CONTEXT.md`) itself — inline nothing — and returns "two engineers would diverge here" findings. Follow the dispatch discipline in [`../_shared/critic.md`](../_shared/critic.md): clean context, Reads upstream itself, cited findings only, `NO_AMBIGUITIES` if none.
